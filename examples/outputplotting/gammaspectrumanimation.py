...
with pp.Reader(filename) as output:
    ...
    if count > 0:
        ...
        energy = [
            (gs.boundaries[i] + gs.boundaries[i + 1]) / 2.0
            for i in range(0, len(gs.boundaries) - 1)
        ]
        values = gs.values
        ...
        def animate(i):
            ...
            energy = [
                (gs.boundaries[j] + gs.boundaries[j + 1]) / 2.0
                for j in range(0, len(gs.boundaries) - 1)
            ]
            values = gs.values
        ...
        ani = animation.FuncAnimation(
            fig, animate, np.arange(0, count), interval=200, blit=True
        )
