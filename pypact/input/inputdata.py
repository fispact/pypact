...
class InputData(JSONSerializable):
    ...
    def readXSData(self, group, binary=False):
        if self._useeaf and binary:
            raise PypactIncompatibleOptionException(
                "Cannot enable binary format reading when using EAF"
            )
    ...
    def useEAFLibraries(self, use=True):
        if self._binaryxs and use:
            raise PypactIncompatibleOptionException(
                "Cannot enable EAF data libraries with binary format"
            )
    ...
    def addElement(self, element, percentage=100.0):
        ...
        if percentage > 100.0:
            raise PypactOutOfRangeException(
                "Cannot set the element percentage above 100%."
            )
    ...
    def _serialize(self, f):
        ...
        if self._readspontfission:
            addcomment(
                "read spontaneous fission from file, specify sf_endf in files file"
            )
            addkeyword("READSF")
        ...
        if not self._ignorecollapse:
            if self._group != 0:
                ...
                addkeyword(
                    "GETXS",
                    args=[
                        -1 if self._binaryxs and not self._useeaf else 1,
                        self._group,
                    ],
                )
            else:
